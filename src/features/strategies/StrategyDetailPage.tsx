...
type BacktestControl = { id: string; symbol?: string; name: string; metrics: Partial<Metrics> };
type BacktestResult = {
  metrics?: Partial<Metrics> & { vsSpy?: number | null };
  controls?: BacktestControl[];
  comparison?: { vsSpyPercent?: number | null; vsIndexPercent?: Record<string, number | null> };
  id?: string;
};
...
export function StrategyDetailPage() {
  ...
  const [visibleMethods, setVisibleMethods] = React.useState<Set<string>>(() => new Set(["strategy", "control/index/spy", "control/index/qqq", "control/index/iwm", "control/index/dia"]));
  ...
  return (
    <div className="strategy-detail page-stack">
      ...
      {testedMethods.length ? <section className="panel tested-methods-panel"><header className="panel-header"><h2>Index-fund controls</h2><span>Choose comparison groups</span></header><div className="control-checks" role="group" aria-label="Visible tested methods">{testedMethods.map((method) => <label key={method.id}><input type="checkbox" checked={visibleMethods.has(method.id)} onChange={() => toggleMethod(method.id)} /><span><strong>{method.id === "strategy" ? "Strategy" : method.symbol ?? method.id}</strong><small>{method.name}</small></span></label>)}</div><div className="table-scroll"><table className="data-table backtest-comparison" aria-label="Backtest method comparison"><thead><tr><th>Metric</th>{visibleResults.map((method) => <th className="numeric" key={method.id}>{method.id === "strategy" ? "Strategy" : method.symbol ?? method.id}</th>)}</tr></thead><tbody>{COMPARISON_METRICS.map((metric) => <tr key={metric}><th>{metric === "returnPercent" ? "Return" : metric === "finalEquity" ? "Final equity" : metric === "maxDrawdown" ? "Max drawdown" : metric === "sharpe" ? "Sharpe" : "Trades"}</th>{visibleResults.map((method) => <td className="numeric" key={method.id}>{formatMetric(metric, method.metrics[metric])}</td>)}</tr>)}</tbody></table></div>{comparison?.vsIndexPercent ? <p className="comparison-summary">{Object.entries(comparison.vsIndexPercent).map(([symbol, value]) => <React.Fragment key={symbol}>Strategy vs {symbol}: <strong>{formatMetric("returnPercent", value)}</strong>{" · "}</React.Fragment>)}</p> : null}</section> : null}
